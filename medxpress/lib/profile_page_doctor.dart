...
class ProfilPageDoctor extends StatefulWidget {
  final int korisnikId;
  final String ime;
  final String prezime;

  const ProfilPageDoctor({
    super.key,
  ...
  @override
  State<ProfilPageDoctor> createState() => _ProfilPageDoctorState();
}

class _ProfilPageDoctorState extends State<ProfilPageDoctor> {
  late String _ime;
  ...
  Future<void> _fetchProfile() async {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        setState(() {
          _ime = data["ime"] ?? _ime;
          _prezime = data["prezime"] ?? _prezime;
        });
      } else {
  ...
  Future<void> _openChangePasswordDialog() async {
    ...
    // varijable za toggle vidljivosti izvan buildera
                ...
                children: [
                  Row(
                    children: [
                      CircleAvatar(
                        radius: 28,
                        child: Text(_initials(_ime, _prezime),
                            style: const TextStyle(
                                fontSize: 18, fontWeight: FontWeight.bold)),
                      ),
                      const SizedBox(width: 16),
                      Expanded(
                        child: Text("$_ime $_prezime",
                            style: const TextStyle(
