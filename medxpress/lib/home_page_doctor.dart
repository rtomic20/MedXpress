...
class HomePageDoctor extends StatefulWidget {
  ...
  /// korisnički ID (Korisnik.pk) za profil akcije
  final int korisnikId;

  /// model ID doktora (Doktor.pk) za termine/filtriranje
  final int doktorId;
...
class _HomePageDoctorState extends State<HomePageDoctor> {
  ...
  Future<void> _fetchPatientsFromConversations() async {
    try {
      ...
      if (res.statusCode == 200) {
        ...
        final doctorConvs = list
            .map((e) => (e as Map).cast<String, dynamic>())
            .where((c) => (c['doktor'] as int?) == widget.doktorId);

        final uniq = <int, Map<String, dynamic>>{};
        for (final conv in doctorConvs) {
        ...
        debugPrint('GET /conversations/ => ${res.statusCode}');
      }
    } catch (e) {
      debugPrint('Greška dohvaćanja pacijenata: $e');
    }
  }
  ...
  Future<void> _fetchAppointments() async {
    try {
      final uri = Uri.parse('$baseUrl/appointments/')
          .replace(queryParameters: {'doktor_id': '${widget.doktorId}'});
      final res = await http.get(uri);
      if (res.statusCode == 200) {
        ...
        for (final a in items) {
          final startIso = (a['start_time'] ?? a['start'])?.toString();
          if (startIso == null || startIso.isEmpty) continue;
          final dt = DateTime.tryParse(startIso);
          if (dt == null) continue;
          setDates.add(_ymd(dt));
        }
        setState(() {
          _appointments
            ..clear()
        ...
        debugPrint('GET /appointments/ => ${res.statusCode}');
      }
    } catch (e) {
      debugPrint('Greška dohvaćanja termina: $e');
    }
  }
  ...
  Future<void> _openAddAppointmentDialog() async {
    ...
    if (_selectedPatient == null) {
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text('Odaberite pacijenta.')));
      return;
    }

    final int pacijentId = _selectedPatient!['korisnik_id'] as int;
  ...
  Future<void> _confirmAndDeleteAppointment(int id) async {
    ...
    if (ok == true) await _deleteAppointment(id);
  }
  ...
  Future<void> _createAppointment({
    ...
    try {
      ...
      if (res.statusCode == 201 || res.statusCode == 200) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Greška (${res.statusCode}): $body')));
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _updateAppointment({
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ScaffoldMessenger.of(context)
            .showSnackBar(const SnackBar(content: Text('Termin je ažuriran.')));
        await _fetchAppointments();
        ...
        child: SingleChildScrollView(
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                Text(
                  'Dobrodošli, dr. ${widget.ime} ${widget.prezime}!',
                ...
                Container(
                  ...
                  child: CalendarCarousel(
                    ...
                    weekdayTextStyle: const TextStyle(
                        fontWeight: FontWeight.w600, color: Color(0xFF4A4F59)),
                    daysTextStyle:
                        const TextStyle(color: Color(0xFF30343A), fontSize: 14),
                ...
                const SizedBox(height: 8),
                Text('Odaberi datum u kalendaru i spremi termin za pacijenta.',
                    textAlign: TextAlign.center,
  ...
  Widget _navItem(BuildContext context, String imagePath, String label,
      VoidCallback onTap) {
    return Column(
      mainAxisSize: MainAxisSize.min,
